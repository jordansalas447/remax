...
export function PropiedadEstatusItem({ revisiones, loading }: DocumentsCheckProps) {
    ...
    console.log(revisiones[0]?.revisiones)

    // Aplanar los items checklist para mostrar como documentos pendientes o revisar
    const allDocuments =
        Array.isArray(revisiones) && revisiones.length > 0 && Array.isArray(revisiones[0]?.revisiones)
            ? revisiones[0].revisiones.map((item: any) => ({
                  id: item.id_revision,
